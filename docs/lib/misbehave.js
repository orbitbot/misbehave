...
module.exports = function () {
  var self = this;
  var on$$1 = interopDefault(require$$0);
  var element = self.element;

  self.eventHandler = interopDefault(require$$0$1).bind(self);

  on$$1(element, 'keypress', self.eventHandler);
  on$$1(element, 'keydown', self.eventHandler);
  on$$1(element, 'keyup', self.eventHandler);
};
});
...
var off$$1 = interopDefault(require$$0).off;
module.exports = function () {
  var self = this;
  var element = self.element;
...
var leadingWhitespace$$1 = utils.leadingWhitespace;
var removeIfStartsWith$$1 = utils.removeIfStartsWith;
var onNewLine$$1 = utils.onNewLine;
var allNewLines$$1 = utils.allNewLines;
var allCharacters$$1 = utils.allCharacters;


var autoIndent = function (newLine, tab, prefix, selected, suffix) {
  var prevLine = prefix.split(onNewLine$$1).splice(-1)[0];
  var prefEnd = prefix.slice(-1);
  var suffStart = suffix.charAt(0);

  if ((prevLine.match(/\(/g) || []).length > (prevLine.match(/\)/g) || []).length) {
    var whitespace = prevLine.match(leadingWhitespace$$1)[0];
    prefix += newLine + whitespace + prevLine.slice(whitespace.length, prevLine.lastIndexOf('(') + 1).replace(allCharacters$$1, ' ');
  } else if (prefEnd === '{') {
    prefix += newLine + prevLine.match(leadingWhitespace$$1)[0] + tab;
    if (suffStart === '}')
      suffix = newLine + prevLine.match(leadingWhitespace$$1)[0] + suffix;
  } else {
    prefix += newLine + prevLine.match(leadingWhitespace$$1)[0];
  }
  selected = '';
...
var tabIndent = function (newLine, tab, prefix, selected, suffix) {
  var prefLines = prefix.split(onNewLine$$1);
  ...
  if (selected === '') {
    ...
    selected = selected.replace(allNewLines$$1, newLine + tab);
  }
  return { prefix: prefix, selected: selected, suffix: suffix }
};

var tabUnindent = function (newLine, tab, prefix, selected, suffix) {
  var lines = selected.split(onNewLine$$1);
  var prevLine = prefix.split(onNewLine$$1).splice(-1)[0];
  ...
  if (lines.length === 1) {
    ...
    prevLine = removeIfStartsWith$$1(tab)(prevLine);
    prefix = prefix.slice(0, -prevLength) + prevLine;
    lines = lines.map(removeIfStartsWith$$1(tab));
...
var Misbehave = function Misbehave(elem, ref) {
  if ( ref === void 0 ) ref = {};
  var autoIndent$$1 = ref.autoIndent; if ( autoIndent$$1 === void 0 ) autoIndent$$1 = true;
  var autoOpen$$1 = ref.autoOpen; if ( autoOpen$$1 === void 0 ) autoOpen$$1 = true;
  var autoStrip$$1 = ref.autoStrip; if ( autoStrip$$1 === void 0 ) autoStrip$$1 = true;
  var overwrite$$1 = ref.overwrite; if ( overwrite$$1 === void 0 ) overwrite$$1 = true;
  ...
  pairs.forEach(function (ref) {
    ...
    if (closing) {
      if (autoOpen$$1)keys.bind(opening, fnAutoOpen(opening, closing));
      if (overwrite$$1) keys.bind(closing, fnOverwrite(closing));
    } else {
      if (autoOpen$$1 && overwrite$$1) {
        ...
        if (autoOpen$$1)keys.bind(opening, fnAutoOpen(opening, opening));
        if (overwrite$$1) keys.bind(opening, fnOverwrite(opening));
      }
    }
  });
